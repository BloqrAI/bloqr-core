...
New to this compiler? [`config.example.jsonc`](config.example.jsonc) is a fully-commented, ready-to-run starter config (compiles EasyList out of the box); [`config.example.json`](config.example.json) is the same config as strict JSON. EasyList mixes cosmetic (browser-engine) rules with plain domain-blocking rules, so this starter routes it to the browser engine explicitly (see the `.jsonc` comments) — compiling it therefore also needs `compile --allow-unvalidated-output` for now, since `bloqr-validate` doesn't yet validate browser syntax natively (tracked separately; see [Dual-Engine Compilation](../../../docs/architecture/dual-engine-compilation.md)).
