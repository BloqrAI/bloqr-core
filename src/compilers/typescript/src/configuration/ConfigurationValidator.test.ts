...
Deno.test('ConfigurationValidator.validate - should reject commercial-only transformations that silently no-op (issue #502)', () => {
  ...
  for (const transformation of [TransformationType.ConflictDetection, TransformationType.RuleOptimizer]) {
    const globalConfig = {
      name: 'Test Filter',
      sources: [{ source: 'https://example.org/list.txt' }],
      transformations: [transformation],
    };
    assertEquals(validator.validate(globalConfig).valid, false, `expected '${transformation}' to be rejected globally`);
