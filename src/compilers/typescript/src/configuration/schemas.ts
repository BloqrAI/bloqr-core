 ...
 */
export const PrioritySchema: z.ZodEnum<{ standard: 'standard'; high: 'high' }> = z.enum([
  'standard',
  'high',
...
const transformationOrderingMessage = {
  message:
    'Deduplicate transformation is recommended before Compress. Add Deduplicate before Compress in transformations.',
...
export const SourceSchema: z.ZodType<ISource> = z.object({
  ...
  name: z.string().trim().min(1, 'name must be a non-empty string').optional().describe(
...
export const ConfigurationSchema: z.ZodType<IConfiguration> = z.object({
  name: z.string().min(1, 'name is required and must be a non-empty string').describe(
    'Filter list name',
  ),
  description: z.string().optional().describe('Human-readable description of the filter list'),
  homepage: z.string().url('homepage must be a valid URL').optional().describe(
    'Homepage URL for the filter list',
  ),
...
export const CompileRequestSchema: z.ZodType<CompileRequestOutput> = z.object({
  ...
  benchmark: z.boolean().optional().describe(
    'Whether to collect benchmark metrics during compilation',
  ),
...
export const BatchRequestSchema: z.ZodType<BatchRequestOutput> = z.object({
  requests: z.array(
    z.object({
      id: z.string().min(1, 'id is required and must be a non-empty string').describe(
        'Unique identifier for the batch request item',
      ),
      configuration: ConfigurationSchema,
      preFetchedContent: PreFetchedContentSchema,
      benchmark: z.boolean().optional().describe(
        'Whether to collect benchmark metrics for this item',
      ),
    }),
...
export const BenchmarkMetricsSchema: z.ZodType<BenchmarkMetricsOutput> = z.object({
  totalDurationMs: z.number().nonnegative().describe(
    'Total end-to-end compilation duration in milliseconds',
  ),
  stages: z.array(z.object({
    name: z.string().describe('Stage name'),
    durationMs: z.number().nonnegative().describe('Stage duration in milliseconds'),
    itemCount: z.number().int().nonnegative().optional().describe(
      'Number of items processed in this stage',
    ),
    itemsPerSecond: z.number().nonnegative().optional().describe(
 ...
 */
export const WorkerCompilationResultSchema: z.ZodType<WorkerCompilationResultOutput> =
  compilationResultBase.extend({
...
export const CliArgumentsSchema: z.ZodType<CliArgumentsOutput> = z.object({
  ...
  maxRules: z.number().int().positive().optional().describe(
    'Truncate output to at most this many rules',
  ),
  ...
  apiKey: z.string().regex(
    /^(blq_|abc_).+$/,
    'API key must start with "blq_" (or legacy "abc_") followed by key material',
  ).optional().describe(
    'API key for authenticated worker API requests (blq_ prefix; legacy abc_ prefix also accepted)',
  ),
  bearerToken: z.string().optional().describe(
    'Better Auth JWT bearer token for authenticated requests',
  ),
...
export const EnvironmentSchema: z.ZodType<EnvironmentOutput> = z.object({
  RATE_LIMIT_MAX_REQUESTS: z.coerce.number().int().positive().optional().describe(
    'Maximum requests per rate-limit window',
  ),
  RATE_LIMIT_WINDOW_MS: z.coerce.number().int().positive().optional().describe(
    'Rate-limit window duration in milliseconds',
  ),
  CACHE_TTL: z.coerce.number().int().positive().optional().describe(
    'Cache time-to-live in seconds',
  ),
  LOG_LEVEL: z.enum(['trace', 'debug', 'info', 'warn', 'error']).optional().describe(
