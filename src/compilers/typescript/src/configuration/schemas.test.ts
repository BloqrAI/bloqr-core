...
Deno.test('BatchRequestSyncSchema - should validate batch with unique IDs', () => {
  const batch = {
    requests: [
      {
        id: '1',
        configuration: { name: 'Test', sources: [{ source: 'https://example.com/1.txt' }] },
      },
      {
        id: '2',
        configuration: { name: 'Test', sources: [{ source: 'https://example.com/2.txt' }] },
      },
    ],
  };
...
Deno.test('CliArgumentsSchema - should fail when neither input nor config is provided', () => {
  ...
  if (!result.success) {
    const messages = result.error.issues.map((i) => i.message);
    assertEquals(
      messages.includes('Either --input or --config must be specified (or --help/--version)'),
      true,
    );
  }
});
...
Deno.test('CliArgumentsSchema - should fail when both config and input are provided', () => {
  ...
  if (!result.success) {
    const messages = result.error.issues.map((i) => i.message);
    assertEquals(
      messages.includes('Cannot specify both config file (-c) and input sources (-i)'),
      true,
    );
  }
});
