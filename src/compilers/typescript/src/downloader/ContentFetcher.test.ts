...
Deno.test('ContentFetcher - maps Node-style ENOENT errors to typed file-not-found errors', async () => {
  // Simulates the NodeFileSystem path: Node/Bun surface missing files as
  // errors with code ENOENT, not Deno.errors.NotFound.
  class EnoentFileSystem implements IFileSystem {
  ...
  const fetcher = new ContentFetcher(mockLogger, undefined, new EnoentFileSystem());

  await assertRejects(
    () => fetcher.fetch('./missing.txt'),
    Error,
    'File not found',
  );
});

Deno.test('ContentFetcher - maps Node-style EACCES errors to typed permission errors', async () => {
  class EaccesFileSystem implements IFileSystem {
    readTextFile(_path: string): Promise<string> {
  ...
  const fetcher = new ContentFetcher(mockLogger, undefined, new EaccesFileSystem());

  await assertRejects(
    () => fetcher.fetch('./forbidden.txt'),
    Error,
    'Permission denied',
  );
});
