...
/**
 * File system implementation backed by Node's `node:fs/promises`.
 *
 * Works identically under Node.js and Bun, since Bun implements Node's `fs`
 * module natively — this is the fallback used whenever the `Deno` global is
 * not present.
 */
...
/**
 * Picks the appropriate {@link IFileSystem} implementation for the current runtime.
 * Deno is preferred; Node.js and Bun both fall through to {@link NodeFileSystem}.
 */
export function detectFileSystem(): IFileSystem {
  return typeof Deno !== 'undefined' ? new DenoFileSystem() : new NodeFileSystem();
}
...
export class ContentFetcher {
  ...
  constructor(
    ...
    this.fileSystem = fileSystem || detectFileSystem();
  ...
  private async fetchFromFile(path: string): Promise<string> {
    try {
      ...
      // Guard the Deno global — this module also runs under Node/Bun via
      // NodeFileSystem, where referencing Deno.errors throws a ReferenceError.
      ...
      if (error instanceof Error) {
        ...
        // Deno throws NotCapable without --allow-read; Node/Bun surface EACCES.
        if (error.name === 'NotCapable' || code === 'EACCES') {
