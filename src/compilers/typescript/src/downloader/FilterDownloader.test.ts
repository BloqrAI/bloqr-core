...
Deno.test('FilterDownloader - custom downloaders', async (t) => {
  await Deno.mkdir(TEST_DIR, { recursive: true });

  await t.step('should use a custom downloader resolved for the source scheme', async () => {
    const downloader = new FilterDownloader({
      resolveCustomDownloader: (source) =>
        source.startsWith('custom://')
          ? { download: () => Promise.resolve(['||from-custom.com^', '||also-custom.com^']) }
          : undefined,
    });

    const result = await downloader.download('custom://bucket/list.txt');

    assertEquals(result, ['||from-custom.com^', '||also-custom.com^']);
  });

  await t.step(
    'should fall through to built-in handling when resolver returns undefined',
    async () => {
      await Deno.writeTextFile(`${TEST_DIR}/fallthrough.txt`, '||builtin.com^\n');

      const downloader = new FilterDownloader({
        resolveCustomDownloader: () => undefined,
      });

      const result = await downloader.download(`${TEST_DIR}/fallthrough.txt`);
      assertEquals(result, ['||builtin.com^']);
    },
  );

  await t.step('should process preprocessor directives in custom-downloaded content', async () => {
    const downloader = new FilterDownloader({
      resolveCustomDownloader: (source) =>
        source.startsWith('custom://')
          ? {
    ...
    assertEquals(result, ['||conditional.com^']);
  });
...
Deno.test('FilterDownloader - concurrent downloads on a shared instance', async () => {
  await Deno.mkdir(TEST_DIR, { recursive: true });

  try {
    // Two lists that both include the same shared file. With instance-level
    // visited-state these concurrent downloads raced: one call cleared the
    // set mid-download and the shared include was misreported as circular.
    await Deno.writeTextFile(`${TEST_DIR}/shared.txt`, '||shared.com^\n');
    await Deno.writeTextFile(`${TEST_DIR}/list_a.txt`, '||a.com^\n!#include shared.txt\n');
    await Deno.writeTextFile(`${TEST_DIR}/list_b.txt`, '||b.com^\n!#include shared.txt\n');

    const downloader = new FilterDownloader();
    const [a, b] = await Promise.all([
      downloader.download(`${TEST_DIR}/list_a.txt`),
      downloader.download(`${TEST_DIR}/list_b.txt`),
    ]);

    assertEquals(a, ['||a.com^', '||shared.com^']);
    assertEquals(b, ['||b.com^', '||shared.com^']);

    // Sequential re-download of the same source must also work
    const again = await downloader.download(`${TEST_DIR}/list_a.txt`);
    assertEquals(again, ['||a.com^', '||shared.com^']);
  } finally {
    await Deno.remove(TEST_DIR, { recursive: true });
  }
});
