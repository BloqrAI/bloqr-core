  ...
  constructor(optionsOrLogger?: FilterCompilerOptions | ILogger) {
    ...
    if (optionsOrLogger && 'info' in optionsOrLogger) {
      ...
      if (options?.hookManager) {
        ...
        options?.events?.onTransformationStart || options?.events?.onTransformationComplete
      ) {
        ...
        resolvedHookManager = new TransformationHookManager(
          createEventBridgeHook(this.eventEmitter),
        );
      } else {
    ...
    this.pipeline = deps?.pipeline ??
      new TransformationPipeline(undefined, this.logger, this.eventEmitter, resolvedHookManager);
