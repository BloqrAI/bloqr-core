...
import { type IConfiguration, SourceType, TransformationType } from '../types/index.ts';
...
Deno.test('FilterCompiler - should add checksum to compiled output', async () => {
  ...
  const firstSourceIndex = result.findIndex((line) =>
    line.includes('Source name:') || line.includes('! Source:')
  );
...
Deno.test('FilterCompiler - should pass downloaderOptions to FilterDownloader.download', async () => {
  ...
  const downloadStub = stub(
    ...
    (
      ...
      _additional?: { allowEmptyResponse?: boolean },
    ) => {
...
Deno.test('FilterCompiler - onCompilationStart fires with correct configName', async () => {
  const startEvents: Array<
    { configName: string; sourceCount: number; transformationCount: number; timestamp: number }
  > = [];
  const compiler = new FilterCompiler({
    logger: silentLogger,
    events: {
