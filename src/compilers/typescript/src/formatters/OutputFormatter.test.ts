...
Deno.test('HostnameListFormatter - custom subclass only needs to define the line format', () => {
  // RPZ-style formatter implemented via the template-method base class:
  // extraction, deduplication, and skip counting are inherited.
  class RpzFormatter extends HostnameListFormatter {
    protected override readonly outputFormat = 'rpz';
  ...
  const formatter = new RpzFormatter({ includeHeader: false });
  const result = formatter.format([...sampleRules, '||ads.example.com^']);

  assertEquals(result.format, 'rpz' as OutputFormat);
  assertEquals(result.ruleCount, 3); // deduplicated blocking rules
  assertEquals(result.content.startsWith('$TTL 300'), true);
  assertEquals(result.content.includes('ads.example.com CNAME .'), true);
  assertEquals(result.content.includes('allowed.example.com'), false);
  // Duplicate rule must not produce a second line
  assertEquals(result.content.match(/ads\.example\.com CNAME \./g)?.length, 1);
});

Deno.test('HostnameListFormatter - header and preamble ordering matches legacy formatters', () => {
  const formatter = new UnboundFormatter({ includeHeader: true, listName: 'Ordering Test' });
  const result = formatter.format(['||ads.example.com^']);
  const lines = result.content.split('\n');

  // Header comment lines first, then the server: preamble, then hostnames
  assertEquals(lines[0], '# Ordering Test');
  assertEquals(lines.includes('server:'), true);
  assertEquals(lines.indexOf('server:') < lines.findIndex((l) => l.includes('local-zone')), true);
});
