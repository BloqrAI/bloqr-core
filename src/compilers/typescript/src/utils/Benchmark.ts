...
export class BenchmarkCollector {
  ...
  public generateReport(): string {
    ...
    // Summary metrics
    lines.push(
      `│  Sources: ${metrics.sourceCount.toString().padStart(10)} │ Input rules: ${
        metrics.ruleCount.toString().padStart(10)
      } │`,
    );
    lines.push(
      `│  Output:  ${metrics.outputRuleCount.toString().padStart(10)} │ Total time:  ${
        formatDuration(metrics.totalDurationMs).padStart(10)
      } │`,
    );
    lines.push('├─────────────────────────────────────────────────────────────┤');
