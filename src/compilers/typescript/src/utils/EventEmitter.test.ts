...
import {
  type IBasicLogger,
  type ICompilationCompleteEvent,
  type ICompilerEvents,
  type IProgressEvent,
  type ISourceCompleteEvent,
  type ISourceErrorEvent,
  type ISourceStartEvent,
  type ITransformationCompleteEvent,
  type ITransformationStartEvent,
  SourceType,
  TransformationType,
} from '../types/index.ts';
...
Deno.test('CompilerEventEmitter - should not throw when no handler registered', () => {
  ...
  emitter.emitTransformationComplete({
    name: TransformationType.Validate,
    inputCount: 0,
    outputCount: 0,
    durationMs: 0,
  });
  emitter.emitProgress({ phase: 'sources', current: 0, total: 1, message: '' });
  emitter.emitCompilationComplete({
    ruleCount: 0,
    totalDurationMs: 0,
    sourceCount: 0,
