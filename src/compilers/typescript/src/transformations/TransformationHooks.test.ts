...
Deno.test('createEventBridgeHook - beforeTransform calls emitTransformationStart', async () => {
  ...
  const mockEmitter = {
    ...
    emitTransformationComplete: (
      _e: { name: string; inputCount: number; outputCount: number; durationMs: number },
    ) => {},
  };
...
Deno.test('createEventBridgeHook - afterTransform calls emitTransformationComplete', async () => {
  const completeEvents: Array<
    { name: string; inputCount: number; outputCount: number; durationMs: number }
  > = [];
  const mockEmitter = {
    emitTransformationStart: (_e: { name: string; inputCount: number }) => {},
    emitTransformationComplete: (
      e: { name: string; inputCount: number; outputCount: number; durationMs: number },
    ) => {
