...
- **lib**: `./lib`'s `RulesCompiler`/`RulesCompilerBuilder`/`RulesCompilerServiceOptions`/`createRulesCompiler` were the one place in the codebase where the pre-#331/#372 "Rules"-branded naming (banned as an organizing/internal noun everywhere else — see `docs/adr/0004-src-reorg-naming-blueprint.md`) survived, because Rust and PowerShell's equivalent hard renames didn't touch this already-published `@bloqr/compiler-core` API surface. Added `BloqrCompiler`/`BloqrCompilerBuilder`/`BloqrCompilerServiceOptions`/`createBloqrCompiler` as the new primary names (`src/lib/bloqr-compiler.ts`, replacing `src/lib/rules-compiler.ts`); the old names remain as `@deprecated` aliases re-exported unchanged from `./lib` and `./orchestration`, so this is not a breaking change. The old names will be removed in the next major version.
...
- **docs**: `deno task lint:docs` now scans every file `deno.json`'s `publish.include` actually ships (everything under `src/`), not just the six `exports`-map entrypoints — the previous version scanned only entrypoint-reachable symbols, which missed real, undocumented exports in files like `src/utils/ErrorUtils.ts` (a second, distinct `ErrorCode` enum) and `src/types/websocket.ts` that are shipped in the published package but never re-exported by name from any entrypoint. Documented all such gaps found (71 across 6 files: `configuration/sources/index.ts`, `downloader/ContentFetcher.ts`, `downloader/PreprocessorEvaluator.ts`, `types/websocket.ts`, `utils/ErrorReporter.ts`, `utils/ErrorUtils.ts`); coverage across the full published file set is 1425/1425 (100%).
...
- **typescript**: stop config validator rejecting ConflictDetection/RuleOptimizer (#510)

## [1.3.0] - 2026-08-23

### Added

- **compiler-core**: add real --benchmark mode, fix broken standard compile

## [1.2.1] - 2026-08-22

### Fixed

- **validation**: fail-closed enforcement across all compilers, retire shell/AdGuardWebhook, split Rust compiler into core/cli (#410)
...
- **rules-validator**: wire rules-validator into Rust, Python, TypeScript, PowerShell, and Shell wrappers (#361)
