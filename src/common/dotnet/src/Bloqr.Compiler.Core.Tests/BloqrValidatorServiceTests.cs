...
/// <summary>
/// Exercises <see cref="BloqrValidatorService"/> against the real native <c>bloqr_validator</c>
...
/// present, since <see cref="BloqrValidatorService"/> is explicitly designed to degrade
/// gracefully rather than require the native library to exist.
/// </summary>
public sealed class BloqrValidatorServiceTests : IDisposable
{
    private readonly string _tempDirectory;
    private readonly BloqrValidatorService _service;

    public BloqrValidatorServiceTests()
    {
        _tempDirectory = Directory.CreateTempSubdirectory("bloqr-validator-service-tests-").FullName;
        _service = new BloqrValidatorService(new Mock<ILogger<BloqrValidatorService>>().Object);
    }
