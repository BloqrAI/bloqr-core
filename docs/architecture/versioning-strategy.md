...
2. **`version.workspace = true` (inheriting `[workspace.package] version`) must not be used for any independently-published or independently-cadenced crate.** Workspace-inherited versioning is the crates.io equivalent of the "one repo, one version" anti-pattern this doc's introduction already rejects for JSR — it would force `bloqr-validator-core`, `bloqr-validator-core-cli`, and `bloqr-compiler` to bump in lockstep with each other, even when only one of them changed. All three now declare an explicit, independent `version` field instead: `bloqr-validator-core` and `bloqr-validator-core-cli` (`src/validation/`) as of #365's follow-up, and `bloqr-compiler` (`src/compilers/rust/`) as of its own crates.io publish decision (reversing #365's original "unpublished, stays workspace-internal" call for this crate specifically).
...
Both `bloqr-validator-core` (library) and `bloqr-validator-core-cli` (the `bloqr-validate` binary — installable via `cargo install`, the standard crates.io distribution mechanism for Rust CLI tools) publish via the same `publish-crates.yml`, triggered on every push to `main` touching either crate's directory (or manually via `workflow_dispatch`) — the same path-filtered-push pattern as `publish-jsr.yml`, gated by an idempotency check against the crates.io API rather than a `cargo publish --skip-duplicate` flag (which doesn't exist). The CLI's job runs after the library's (`needs: publish-core`), since `bloqr-validator-core-cli`'s dependency on `bloqr-validator-core` is rewritten from a `path` dependency to a registry one at publish time (`version = "1"` alongside `path` in its `Cargo.toml`) — a brand-new library version needs to exist on the registry before the CLI can resolve against it.

This is deliberately **not** part of `release.yml`'s repo-wide `v*`-tag-triggered binary release: `bloqr-compiler`'s release *binary* still bundles into that coordinated multi-language GitHub Release (it isn't published to crates.io at all — no external consumer as a library, and the binary already ships there), but neither crates.io publish needs to wait for (or force) a full binary release cut just because a validation-logic or CLI-only change landed. `docs/architecture/nuget-distribution-strategy.md`'s `publish-nuget.yml` got the identical treatment for the same reason.

What's still genuinely manual: there's no automated Conventional-Commits bump/tag workflow pair (the `compiler-core-version-bump.yml`/`compiler-core-create-version-tag.yml` equivalents) for either crate yet — today their versions are bumped by hand in `Cargo.toml`. Building that automation is tracked as follow-up under #372, not this doc.
