...
2. **`version.workspace = true` (inheriting `[workspace.package] version`) must not be used for any independently-published or independently-cadenced crate.** Workspace-inherited versioning is the crates.io equivalent of the "one repo, one version" anti-pattern this doc's introduction already rejects for JSR — it would force `bloqr-validator-core` and `bloqr-validator-core-cli` to bump in lockstep with each other and with `rules-compiler`, even when only one of them changed. As of #365's follow-up, both `bloqr-validator-core` and `bloqr-validator-core-cli` (`src/validation/`) declare an explicit, independent `version = "1.0.0"` instead. `rules-compiler` (`src/rules-compiler-rust/`) is unpublished and workspace-internal, so it stays on `version.workspace = true` for now — revisit if it's ever published independently.
